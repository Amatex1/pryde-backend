...
router.get('/list', auth, requireActiveUser, async (req, res) => {
  try {
    ...
    const populatedConversations = conversations.map(conv => {
      ...
      // Decrypt last message if needed (with backward compatibility for JSON strings and raw hex)
      if (conv.lastMessage?.content) {
        try {
          let contentToDecrypt = conv.lastMessage.content;

          // Handle string format - could be plain text, JSON string, or raw hex encrypted data
          if (typeof conv.lastMessage.content === 'string') {
            // First check if it's a raw hex encrypted string (long hex string without JSON structure)
            if (conv.lastMessage.content.length > 50 && /^[a-f0-9]+$/i.test(conv.lastMessage.content)) {
              // This is a raw encrypted hex string - pass directly to decryptMessage
              ...
              try {
                ...
                // Not JSON, not hex - assume plain text
              }
            }
          }
          // Content is already an object (from aggregation)
      ...
      if (msgObj.content) {
        try {
          ...
          if (typeof msgObj.content === 'string') {
            // First check if it's a raw hex encrypted string
            if (msgObj.content.length > 50 && /^[a-f0-9]+$/i.test(msgObj.content)) {
              // This is a raw encrypted hex string - pass directly to decryptMessage
              contentToDecrypt = msgObj.content;
            } else {
              // Try to parse as JSON for backward compatibility
              try {
